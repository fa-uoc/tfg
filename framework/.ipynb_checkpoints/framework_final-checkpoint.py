...
class Preprocessor:
    ...
    def _pitch_shift(self,waveform: torch.Tensor):
        n_steps = random.uniform(-2.0,2.0) # Bajamos o subimos hasta un tono como máximo
        output= torchaudio.transforms.PitchShift(sample_rate = self.sample_rate,
                                                  n_steps = n_steps)(waveform)
        return output.to(self.device)
    ...
    def transform_batch(self, batch_waves: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "training":
            ...
            for wave in x:
                ...
                p = self.augmentation_probs.get('pitch_shift', 0.0)
                if random.random() < p:
                    wave = self._pitch_shift(wave)
        ...
        if self.mode == "training":
            ...
            if random.random() < p:
                mels = self._frequency_masking(mels)

            p = self.augmentation_probs.get('time_masking', 0.0)
            if random.random() < p:
                mels = self._time_masking(mels)

        mels = mels.unsqueeze(1)

        return mels        #forma x: [BATCH, CHANNEL, TIME]
